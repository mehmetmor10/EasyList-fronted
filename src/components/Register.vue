<template>
  <div class="auth-container">
    <div class="auth-card">
      ...
      <p class="auth-subtitle">Erstelle deinen kostenlosen Account</p>

      <div class="form-group">
        <label>Benutzername</label>
        <input v-model="username" type="text" placeholder="Wähle einen Benutzernamen" />
      </div>

      <div class="form-group">
        <label>Email</label>
        <input v-model="email" type="email" placeholder="Deine Email-Adresse" />
      </div>

      <div class="form-group">
        <label>Passwort</label>
        <input v-model="password" type="password" placeholder="Wähle ein Passwort" />
      </div>

      <div class="form-group">
        <label>Passwort bestätigen</label>
        <input v-model="passwordConfirm" type="password" placeholder="Passwort wiederholen" />
      </div>
      ...
      <p class="auth-switch">
        Bereits ein Account?
        <span @click="$emit('switchToLogin')">Jetzt anmelden</span>
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async register() {
      ...
      this.loading = true
      try {
        const response = await fetch('https://easylist-backend.onrender.com/auth/register', {
          method: 'POST',
        ...
        setTimeout(() => { this.$emit('switchToLogin') }, 2000)
      } catch (error) {
